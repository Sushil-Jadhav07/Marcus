...
const FMCGSectorBeacon = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('FMCG sector fetch failed', e);
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      ws.addEventListener('open', () => {
        try {
          ws.send(JSON.stringify({ action: 'subscribe', channel: 'ohlc', exchange: 'NSE', sector: 'FMCG' }));
        } catch {}
      });
...
export default FMCGSectorBeacon;
