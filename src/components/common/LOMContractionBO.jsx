...
const LOMContractionBO = (
  ...
  const CACHE_KEY = `cache_LOMContraction_${title}`;
    ...
    const fetchData = async () => {
      ...
      try { localStorage.setItem(CACHE_KEY, JSON.stringify(Array.isArray(items) ? items : [])); localStorage.setItem(`${CACHE_KEY}_ts`, Date.now().toString()); } catch (_) {}
        ...
        console.error('LOMContractionBO fetch failed', e);
  ...
  useEffect(() => {
    ...
    const getLow = (it) => {
      const candidates = [it?.low, it?.dayLow, it?.l];
      const val = candidates.find((v) => Number.isFinite(Number(v)));
      return Number(val);
    };

    const getHigh = (it) => {
      const candidates = [it?.high, it?.dayHigh, it?.h];
      const val = candidates.find((v) => Number.isFinite(Number(v)));
      return Number(val);
    };
  ...
  return (
    ...
    <div className="flex items-center gap-4 mb-2">
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-white/70 font-medium">Band Contraction &lt; 4% of Close</div>
      </div>
    ...
    <div className="grid grid-cols-5 text-[15px]  text-black/70 px-2">
      <div className="bg-white/50 px-5 py-2 rounded-full col-span-3">Name</div>
...
export default LOMContractionBO
