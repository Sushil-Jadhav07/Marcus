...
const LOMLongterm = (
  ...
  const CACHE_KEY = `cache_LOMLongterm_${title}`;
    ...
    const fetchData = async () => {
      setIsLoading(true);
      setError(null);
      // If offline, show cached data immediately
      ...
      try {
        ...
        if (cached) { setRawData(JSON.parse(cached)); return; }
      } catch (_) {}
      setRawData([]);
      } finally {
  ...
  useEffect(() => {
    ...
    const data = useMemo(
      () => {
        ...
        const getPct = (it) => toNum(it?.per_chg ?? it?.percentChange ?? it?.pct ?? it?.changePercent);
        return arr.filter((it) => getPct(it) > 0).slice(0, limit);
      },
      [rawData, limit]
    );
