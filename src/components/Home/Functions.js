      ...
      <div className="grid md:grid-cols-6 grid-cols-2 gap-6">
        {features.map(({ title, to, icon }) => (
            <Link
                key={to}
                to={to}
                className="relative h-[140px] backdrop-blur-lg rounded-xl border-t-2 border-r-2 border-b-2 border-l-2 border-t-white/60 border-r-white/60 border-b-blue-400/60 border-l-blue-400/60 w-full flex flex-col items-center justify-center gap-3 bg-white/20 dark:bg-black/20"
                >
                <div className="w-[75px] h-[59px] flex items-center justify-center rounded-xl">
                    <img src={icon} alt={title} className="w-[71px] h-[59px]" />
                </div>
