...
const MobileStrategyBox = ({ title, children, className = "" }) => {
  ...
  useEffect(() => {
   ...
   const processed = useMemo(() => {
     ...
     const baseItems = data
       ...
       .map((item, idx) => {
         const label = item?.nsecode || item?.symbol || item?.ticker || `Item ${idx + 1}`;
     ...
     const rowsTarget = 5; // reduced number of rows for better arrangement
    ...
    <div className={`overflow-y-auto scrollbar-hide bg-gradient-to-br from-blue-900/30 via-blue-800/20 to-indigo-900/30 border-blue-400/40 mt-2 lg:relative backdrop-blur-xl rounded-2xl border-t-2 border-r-2 border-b-2 border-l-2 border-t-white/70 border-r-white/70 border-b-blue-400/70 border-l-blue-400/70 w-full flex flex-col p-6 gap-4 bg-white/25 dark:bg-white/25 shadow-2xl transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/30 h-[500px]`}>
      <div className='flex justify-start items-start gap-5'>
        ...
        <div className="flex-1">
            <h3 className="font-bold tracking-wide text-[18px] dark:text-white text-black drop-shadow-sm">{title}</h3>
        </div>
      </div>
      ...
      {!isLoading && !error && data && (
         <div className="w-full flex-1">
           {Array.isArray(data) && processed ? (
             <div
               className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-9 grid-flow-col-dense  h-full"
               style={{ gridTemplateRows: `repeat(${processed.rowsCount}, minmax(0, 1fr))` }}
             >
               {processed.tiles.map((it) => (
                 <div
                   key={it.key}
                   className={`relative group flex items-center justify-center p-1.5  ${it.filler ? 'opacity-0' : 'cursor-pointer hover:brightness-110'}`}
                  ...
                  onClick={() => { if (!it.filler && it.url) { window.location.href = it.url; } }}
                 >
                   {!it.filler && (
                     <>
                       <span className="text-white font-semibold tracking-wide uppercase text-[10px] sm:text-[11px] md:text-[12px] select-none text-center truncate w-full px-1">
                         {it.label}
                       </span>
...
export default MobileStrategyBox